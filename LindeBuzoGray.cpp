...
//определяет принадлежность векторов данных кластерам и возвращает массив номеров соотв. кластеру
vector < vector <int> > TLindeBuzoGray::SetClasterPoint(vector < vector <double> >& SeqCoff, RModelStage Mdl)
{
    ...
    //fix убить кластер с маленьким количеством точек
    return ClsPoint;
}
//---------------------------------------------------------------------------
...
//---------------------------------------------------------------------------
//--------------------------------------------------------------------------- 
//модификация алгоритма Линде, Бузо и Грея.
//находится дисперсии по каждой оси для всех найденных центроидов
//среди всех дисперсий всех центроидов находится максимальная
//из центроида обладающего максимальной дисперсией делается пара по компоненте соответствующей макс. дисперсии
//положение полученных точек оптимизируется алгоритмом К-mean
